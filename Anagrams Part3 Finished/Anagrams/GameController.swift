...
class GameController: TileDragDelegateProtocol {
    ...
    var audioController: AudioController    //AudioController 객체
    ...
    //hint 버튼 액션 메소드 추가
    //hint 버튼을 클릭해도 "Help"가 콘솔에 표시되지 않음
    //HUDView init() 끝에 self.userInteractionEnabled = false;로 했기 때문
    //HUDView init() 메소드 뒤... hint 버튼이 동작하도록 true로 바꾸고 custom 메소드 hitTest를 구현해야 함
    @objc func actionHint() {
        //1. hint 버튼 disable
        hud.hintButton.isEnabled = false
        //2. hint 버튼 사용하면 점수를 잃음
        data.points -= level.pointsPerTile / 2
        hud.gamePoints.setValue(newValue: data.points, duration: 1.5)
        
        //3. 타겟중에서 아직 match되지 못한 것을 찾음
        ...
        if let target = foundTarget, let tile = foundTile {
            //5. tileView를 맨 위로 올리고
            gameView.bringSubviewToFront(tile)
            //6. tileView를 맞는 target 위치로 animation
            UIView.animate(
                withDuration: 1.5,
                delay: 0.0,
    ...
    init() {
        ...
        tiles = []
        targets = []
        //AudioController 객체를 생성하고 오디오 파일 preload해서 dictionary에 넣는다.
    ...
    func dealRandomAnagram() {
        ...
        //dealRandomAnagram() 메소드 뒤에... hint 버튼 enable
    ...
    func placeTile(tileView: TileView, targetView: TargetView) {
        ...
        //placeTile() 메소드 맨 뒤에.. 타일을 놓은 center에 ExplodeView 생성
    ...
    func checkForSuccess() {
        ...
        //checkForSuccess() 메소드 뒤에.. 왼쪽에서 오른쪽으로 stardustView 파티클 생성
        ...
        UIView.animate(withDuration: 3.0,
            ...
            //checkForSuccess()메소드 stardustView 애니메이션에 clearBoard호출하고 메뉴를 다시 표시
            self.clearBoard()
            self.onAnagramSolved()
        })
        //게임종료이므로 hint 버튼 disable
        hud.hintButton.isEnabled = false
    }
    var onAnagramSolved: (() -> ())!
